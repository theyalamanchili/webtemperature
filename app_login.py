...
# -----------------------------------------------
# 3. Temperatures & Convection (with invertible 1-D)
# -----------------------------------------------
...
# Collect inputs, leaving exactly one None
if missing == "– none –":
    ...
    Tout = st.sidebar.number_input("Outlet temp, T_out at x=L (°C)", -50.0, 500.0, 100.0)
    v    = st.sidebar.number_input("Web speed, v (m·s⁻¹)", 0.01, 10.0, 1.6)
    h    = st.sidebar.number_input("Convective coef, h (W·m⁻²·K⁻¹)", 1.0, 1e4, 100.0)
    T0   = None
...
with st.sidebar.expander("5. Default Parameters", expanded=False):
    st.markdown("Number of eigenmodes, N (–)")
...
if st.sidebar.button("Compute"):
    # collapse the Read-Me
    st.session_state.readme_expanded = False

    # If convective span and one missing, invert 1-D
    if scenario == "Free span convective cooling" and missing != "– none –":
        ...
        # Inlet temperature analytic inverse
        if missing == "Inlet temperature (T₀)":
            μ = mu_of(v, h)
            T0 = Tinf + (Tout - Tinf)*np.exp(-μ * L)
            st.info(f"🔄 Predicted inlet temperature: T₀ = {T0:.2f} °C")
        ...
        # Solve for v
            ...
            st.info(f"🔄 Predicted web speed: v = {v:.3f} m/s")
        ...
        # Solve for h
            ...
            st.info(f"🔄 Predicted convection coeff: h = {h:.1f} W/m²·K")

    # Now compute normally
    if scenario == "Free span convective cooling":
